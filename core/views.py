...
class PedidoViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializers):
        serializers.save(cliente=self.request.user)
        productos = serializers.validated_data.get('productos', [])
        total = sum([p.precio for p in productos])
        serializers.save(cliente=self.request.user,total=total)
...
def post(self, request):
    ...
    'total_pedido': total,
    'faltante': round(total - cliente.saldo, 2)}, status=400)
